...
abstract class WorkspacedResourcePolicy extends BasePolicy
{
    ...
    public function destroy(User $currentUser, $resourceIds = null): bool
    {
        [, $permission] = $this->parseNamespace($this->resource, 'delete');

        if ($this->userHasPermission($currentUser, $permission)->allowed()) {
            return true;
        } elseif ($resourceIds) {
    ...
    protected function userHasPermission(
        ...
        if (
            $userOwnsWorkspace &&
            parent::userHasPermission($user, $permission)->denied()
        ) {
    ...
    private function userIsWorkspaceMember(User $user): bool
    {
